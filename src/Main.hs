...
main = do
  ...
  blankLine
  let dayWeights = filledDays savedWeights
  displayRunningAverage dayWeights
  blankLine
  displayMovingAverages dayWeights
...
displayFromLastWeek es = do
  ...
  displayEntries heading last8Days
...
displayRunningAverage :: [(Day, Weight)] -> IO ()
displayRunningAverage dayWeights =
  let lastNWeeks n = L.drop (length dayWeights - (n * 7)) dayWeights
      dayWeightAvg dws = average (fromWeight . snd <$> dws)
      process n = let range = lastNWeeks n
                      weightAvg = dayWeightAvg range
                      earliestDay = fst (head range)
                  in putStrLn (shortPrettyPrintTime earliestDay <> ": " <> show weightAvg)
      -- printWeekAvg n = putStrLn (show n <> ": " <> show (nWeekAvg n))
  in do
    currentDay <- utctDay <$> getCurrentTime
    printHeading "Average Since __"
    -- forM_ [1..4] printWeekAvg
    forM_ [1..4] process
 where
  leftFolder
    :: (Day, Maybe Weight) -> (Day, Maybe Weight) -> (Day, Maybe Weight)
...
-- FIXME change this from Int to some kind of duration
...
movingAverages n es =
  let week = 7
      ...
      windowAverage = precision 1 . (/ fromIntegral windowSize) . sum . fmap (fromWeight . snd)
      windowResult window = (fst (head window), fst (last window), windowAverage window)
